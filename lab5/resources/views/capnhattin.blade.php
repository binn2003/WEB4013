...
<body>
    <form action="/tin/capnhat/{{$t->id}}" method="post" class="col-7 m-auto p-3">
        <h1 class="text-center">Cập nhật tin</h1>
        <p> ID: <input name="id" class="form-control" value="{{$t->id}}" readonly></p>
        <p> Tiêu đề: <input name="tieuDe" class="form-control" value="{{$t->tieuDe}}" ></p>
        <p> Tóm tắt:
            <input name="tomTat" class="form-control" value="{{$t->tomTat}}" >
        </p>
        ...
        <p><button type="submit" class="btn btn-success p-2">Cập nhật tin</button></p>
        @csrf
    </form>
